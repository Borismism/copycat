...
class GeminiExpandedConfig(BaseModel):
    ...
    high_priority_keywords: list[str] = Field(..., description="🔴 HIGH: Character + 'ai' generic keywords (scan every 3 days)")
    medium_priority_keywords: list[str] = Field(..., description="🟡 MEDIUM: Character + top AI tools - Sora, Runway, Kling, Pika (scan every 7 days)")
    low_priority_keywords: list[str] = Field(..., description="🟢 LOW: Character + niche AI tools - Luma, Veo, Minimax, Gen-2, Gen-3 (scan every 14 days)")
...
## 1. SEARCH KEYWORDS - PRIORITIZED CHARACTER × AI TOOL MATRIX

🚨 **GENERATION STRATEGY: Create prioritized character + AI tool combinations** 🚨
...
- 10 main characters × 11 AI tools = **110 keywords**
  - 10 high priority (character + ai)
  - 40 medium priority (character + top 4 tools)
  - 60 low priority (character + remaining tools)
- 5 franchise properties × (1 high + 2 medium) = **15 keywords**
- 10 locations/objects × "ai" = **10 keywords** (all high priority)
...
- **high**: 1.5 (aggressive monitoring) ← USER SELECTED
...
- **low** → "selective" (lower priority, higher threshold)
- **medium** → "balanced" (standard monitoring)
- **high** → "aggressive" (scan frequently, low threshold) ← USER SELECTED
- **critical** → "aggressive" (maximum protection)
...
async def suggest_keywords(request: SuggestRequest):
    """Suggest additional search keywords for YouTube discovery using CHARACTER × AI TOOL matrix."""
...
🚨 **MANDATORY: Generate FULL CHARACTER × AI TOOL MATRIX** 🚨
...
## Step 2: Generate FULL Character × AI Tool Matrix
...
**MANDATORY PATTERN:**
If you have 5 characters, you MUST generate 5 × 11 = 55 keywords minimum
...
2. If you identify 3 new characters, generate 3 × 11 = 33 keywords
...
REMINDER: Generate the FULL matrix - every character × every AI tool!"""
